...
class OssImageOperator(private val baseUrl: String) : ImageOperator {
    ...
    override fun format(format: String): ImageOperator {
        operations.add("format,$format")
        return this
    }

    override fun build(): String {
        if (operations.isEmpty()) return baseUrl
        val paramString = "x-oss-process=image/${operations.joinToString("/")}"
    ...
    companion object {
        private val pattern = Regex(".*\\.aliyuncs\\.com.*")
        fun matches(url: String) = pattern.matches(url)
        fun create(url: String) = OssImageOperator(url)
    }
}
